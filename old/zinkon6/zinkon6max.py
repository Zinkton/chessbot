...
def get_detailed_score(board, evaluation):
	...
	if board.has_white_castled:
		white_score += 0.5
	if board.has_black_castled:
		black_score += 0.5
	...
	for white_knight in white_knights:
		...
		if board.has_white_castled or (rank > 1 and castle_w):
			piece_value += 25
	...
	for black_knight in black_knights:
		...
		if board.has_black_castled or (rank < 8 and castle_b):
			piece_value += 25
...
def is_leaf(board, depth, max_depth):
	if board.is_checkmate():
		...
		result = get_piece_score(board)
		result += get_detailed_score(board, result)
		return (True, result)
	return (False, 0)
...
def RecSSS(node, board, depth, max_depth):
	...
	if node[1] == nodestatus.UNEXPANDED:
		...
		for x in range(len(legal_moves)):
			...
			is_castling_move = check_castling(board)
			has_white_castled = board.has_white_castled
			has_black_castled = board.has_black_castled
			if board.turn:
				board.has_black_castled = is_castling_move
			else:
				board.has_white_castled = is_castling_move
			if is_leaf_bool(board, depth + 1, max_depth):
				...
				if s_legal_moves:
					...
					node[3].append(son)
			board.pop()
			board.has_white_castled = has_white_castled
			board.has_black_castled = has_black_castled
...
def process_scores(board, move_scores, max_depth):
	global last_fen
	board.has_white_castled = False
	board.has_black_castled = False
	root_node = ['root', nodestatus.UNEXPANDED, 10**11, [], False, False, False]
	while root_node[1] != nodestatus.SOLVED:
		root_node[2] = RecSSS(root_node, board, 0, max_depth)
	results = []
	for result in root_node[3]:
		if result[2] == root_node[2] and result[1] == nodestatus.SOLVED:
			if result[4][0] == 'root':
				results.append([result[0], root_node[2]])
			else:
				results.append([result[4][0], root_node[2]])
...
def process_score(input):
	...
	if board.is_checkmate():
		...
		for move in board.legal_moves:
			...
			process_score([board, new_move_score, depth + 1, max_depth, alpha, beta])
			board.pop()
			if new_move_score[1] == move_score[1]:
				results.append(new_move_score)
			elif board.turn and new_move_score[1] > move_score[1]:
				move_score[1] = new_move_score[1]
				results = [new_move_score]
				if move_score[1] > beta:
					break
				alpha = move_score[1]
...
def check_castling(board):
	if board.turn and board.has_black_castled or not board.turn and board.has_white_castled:
		return True
	
	last_move = board.peek()
	destination_piece = board.piece_at(last_move.to_square)
	return destination_piece.piece_type == chess.KING and abs(last_move.from_square - last_move.to_square) == 2