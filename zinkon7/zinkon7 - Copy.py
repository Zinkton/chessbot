...
def is_leaf(node, board, depth, max_depth):
	if board.is_checkmate():
		...
		white_score = node[7][0]
		black_score = node[7][1]
		if white_score > black_score:
			result = min(9999999999.0, (white_score - black_score) * (white_score / max(1.0, black_score))) / 10.0
		else:
			result = max(-9999999999.0, (white_score - black_score) * (black_score / max(1.0, white_score))) / 10.0
...
def RecSSS(node, board, depth, max_depth):
	...
	if node[1] == nodestatus.UNEXPANDED:
		...
		for x in range(len(legal_moves)):
			sv = node[7][0] + piece_value.get(board.piece_type_at(legal_moves[x].to_square), 0)
			son = [legal_moves[x], nodestatus.UNEXPANDED, node[2], [], node, legal_moves, x, [sv, node[7][1]]]
			...
			if is_leaf_bool(board, depth + 1, max_depth):
				...
				if s_legal_moves:
					gsv = node[7][1] + piece_value.get(board.piece_type_at(s_legal_moves[0].to_square), 0)
					gson = [s_legal_moves[0], nodestatus.UNEXPANDED, node[2], [], son, s_legal_moves, 0, [sv, gsv]]
					node[3].append(gson)
				else:
					node[3].append(son)
			board.pop()
	...
	while gson[2] == node[2] and gson[1] != nodestatus.SOLVED:
		...
		if is_gson and gson[1] == nodestatus.SOLVED and gson[6] != (len(gson[5]) - 1):
			...
			gson[3] = []
			gson[7][1] = gson[4][7][1] + piece_value.get(board.piece_type_at(gson[0].to_square), 0)
		board.pop()
		gson = max(node[3], key=lambda s: s[2])
...
def solve_position(input):
	...
	(result, vw, vb) = get_piece_score(board)
	vb += piece_value.get(board.piece_type_at(move.to_square), 0)
	board.push(move)
	root_node = [move, nodestatus.UNEXPANDED, 10**11, [], False, False, False, [vw, vb]]
