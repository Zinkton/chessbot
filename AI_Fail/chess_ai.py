...
from sss_algorithm_ai import solve_position
...
class ChessAI():
    ...
    def _select_random_best_move(self, board):
        ...
        best_moves = self._evaluate_and_filter_best_moves(
            board, unevaluated_moves, constants.MAX_DEPTH_AI)
        stop = perf_counter()
    ...
    def _evaluate_and_filter_best_moves(self, board, unevaluated_moves, max_depth):
        ...
        agent.eval()
        solve_position_params = [[board.copy(), max_depth, move, model_utilities.copy_model(agent)] for move, _ in unevaluated_moves]
