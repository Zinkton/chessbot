...
def process_scores(board, move_scores, max_depth):
	...
	#results = process_score([board, evaluation, 1, max_depth, -10**11, 10**11])
	...
	with Pool(processes=8,initializer=initProcess,initargs=(alpha_beta,)) as p:
		...
		for x in range(len(move_scores)):
			...
			board_copy.has_white_castled = False
			board_copy.has_black_castled = False
			is_castling_move = check_castling(board_copy)
			if board_copy.turn:
				board_copy.has_black_castled = is_castling_move
			else:
				board_copy.has_white_castled = is_castling_move
...
def process_score(input):
	...
	if board.is_checkmate():
		...
		move_score[1] += get_detailed_score(board, move_score[1])
	else:
		if board.turn:
			move_score[1] = -10**11
		else:
