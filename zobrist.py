    ...
    if board.kings & chess.BB_SQUARES[move.from_square]:
        ...
        if abs(diff) > 1:
            ...
            if diff < 0:
                ...
                rook_square_target = chess.F1 if board.turn else chess.F8
            else:
                # Queen castling
                rook_square_source = chess.A1 if board.turn else chess.A8
                rook_square_target = chess.D1 if board.turn else chess.D8
    ...
    if move.promotion:
        ...
        if src_piece_type == chess.PAWN:
            ...
            if abs(diff) == 16:
                ...
                en_passant_file = chess.square_file(move.from_square)
                new_hash ^= POLYGLOT_RANDOM_ARRAY[772 + en_passant_file]
